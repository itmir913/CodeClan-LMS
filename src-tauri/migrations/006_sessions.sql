-- ── 수행평가 시험 세션 ────────────────────────────────────────────────────────
-- 수행평가의 실제 시험 실행 단위.
-- 같은 수행평가(class_assessment)에 여러 세션 가능 (재시험 등).
-- parent_session_id: 재시험인 경우 원본 세션을 가리킴.
--
-- 상태 머신: WAITING → RUNNING ⇄ PAUSED → CLOSED
--   WAITING : 세션 생성됨. 학생 로그인 시 자동 대기실 입장.
--   RUNNING : 교사가 시작 버튼 → 타이머 가동.
--   PAUSED  : 교사 일시정지. 타이머 멈춤.
--   CLOSED  : 시간 종료(자동) 또는 교사 강제 종료.
--
-- 타이머 계산:
--   남은 시간 = time_limit_min * 60000 - elapsed_ms - (now - started_at)
--   PAUSED 시: elapsed_ms += (paused_at - started_at)
--   RUNNING 재개 시: started_at 갱신, elapsed_ms 누적값 유지
--
-- target_type:
--   ALL        : 수업 전체 학생 대상 (일반 시험)
--   INDIVIDUAL : 특정 학생만 대상 (재시험). assessment_session_targets에 목록 저장.

CREATE TABLE IF NOT EXISTS assessment_sessions (
    id                   INTEGER PRIMARY KEY,
    class_assessment_id  INTEGER NOT NULL REFERENCES class_assessments(id) ON DELETE CASCADE,
    parent_session_id    INTEGER REFERENCES assessment_sessions(id),
    status               TEXT NOT NULL DEFAULT 'WAITING'
                             CHECK (status IN ('WAITING', 'RUNNING', 'PAUSED', 'CLOSED')),
    target_type          TEXT NOT NULL DEFAULT 'ALL'
                             CHECK (target_type IN ('ALL', 'INDIVIDUAL')),
    time_limit_min       INTEGER CHECK (time_limit_min IS NULL OR time_limit_min > 0),
    started_at           TEXT,
    elapsed_ms           INTEGER NOT NULL DEFAULT 0 CHECK (elapsed_ms >= 0),
    paused_at            TEXT,
    closed_at            TEXT,
    created_at           TEXT NOT NULL DEFAULT (datetime('now'))
);

-- ── 재시험 개별 대상 ──────────────────────────────────────────────────────────
-- target_type = 'INDIVIDUAL'인 세션에만 사용.
-- 해당 학생만 세션에 입장 가능.

CREATE TABLE IF NOT EXISTS assessment_session_targets (
    session_id INTEGER NOT NULL REFERENCES assessment_sessions(id) ON DELETE CASCADE,
    student_id INTEGER NOT NULL REFERENCES students(id) ON DELETE CASCADE,
    PRIMARY KEY (session_id, student_id)
);

-- ── 인덱스 ────────────────────────────────────────────────────────────────────

CREATE INDEX IF NOT EXISTS idx_assessment_sessions_class_assessment ON assessment_sessions(class_assessment_id);
CREATE INDEX IF NOT EXISTS idx_assessment_sessions_status           ON assessment_sessions(status);
CREATE INDEX IF NOT EXISTS idx_assessment_sessions_parent           ON assessment_sessions(parent_session_id);
CREATE INDEX IF NOT EXISTS idx_assessment_session_targets_student   ON assessment_session_targets(student_id);
